...
module.exports = function(roto) {
	...
	roto.defineTask('s3', function(callback, options, target, globalOptions) {
		...
		roto.notice(colorize(folder, 'white') + ' -> ' + colorize(options.bucket, 'white') + ' (S3)\n');
		...
		async.series({
			...
			var interval_display = setInterval(function() {
				roto.notice('\r  ' + colorize(Math.round(bytes_uploaded / bytes_queued * 100).toString() + '%', 'green') + ' - ' + Math.round(bytes_uploaded / 1024) + 'kb of ' + Math.round(bytes_queued / 1024) + 'kb            ');
			}, 1000);
