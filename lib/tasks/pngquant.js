...
module.exports = function(roto) {
	...
	roto.defineTask('pngquant', function(callback, options, target, globalOptions) {
		...
		// optipng - MUST BE INSTALLED GLOBALLY (see docs)
		var runOptipng = function(files) {
			var optipng = exec('optipng ' + (files.join(' ')), function(err, stdout, stderr) {
				if (err) {
					roto.error(colorize('OptiPNG Error: ', 'red') + err +  '\n');
		...
		var proc = exec(options.binary + ' ' + args.join(' '), function(err, stdout, stderr) {
			if (err) {
				roto.error(colorize('PNGquant Error: ', 'red') + err +  '\n');
				return callback(false);
			} else {
				roto.notice(stderr + '\n');
				if (options.optipng) {
					runOptipng(options.files);
				}
				callback();
			}
		});
